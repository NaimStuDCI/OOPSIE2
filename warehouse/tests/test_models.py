...
class TestItem(unittest.TestCase):
    ...
    def test_is_expired_true(self):
        item = Item("milk", expiration_date="2000-01-01")
        self.assertTrue(item.is_expired(today=date(2023, 1, 1)))

    def test_is_expired_false(self):
        item = Item("yogurt", expiration_date="2099-12-31")
        self.assertFalse(item.is_expired(today=date(2023, 1, 1)))

    def test_update_values(self):
        item = Item("soda", quantity=5, expiration_date="2023-08-01", price=1.50)
        item.update(quantity=10, price=2.0)
        self.assertEqual(item.quantity, 10)
        self.assertEqual(item.price, 2.0)
        self.assertEqual(item.expiration_date, "2023-08-01")  # unchanged

class TestWarehouse(unittest.TestCase):

    def setUp(self):
        self.wh = Warehouse()
        self.item1 = Item("apple", quantity=5, expiration_date="2023-01-01", price=0.5)
        self.item2 = Item("banana", quantity=10, expiration_date="2026-01-01", price=0.3)
    ...
    def test_sort_by_price(self):
        sorted_items = self.wh.get_sorted_items("price")
